...
main(int argc, char *argv[])
{
	...
	host = argv[1];
	port = "21";   //server 用于control connection的well-known port number 
	...
	*****************************************************************/
	...
	fd = cliopen(host, port); //创建control connection的TCP连接 
...
int
cliopen(char *host, char *port)  //根据指定的ip端口号， 由clinet向server发起一个TCP连接，可以是control connect, 可以是data connect 
{
	/*************************************************************
	//2. code here 
	*************************************************************/
...
*/
void
strtosrv(char *str, char *host, char *port)   //被动：server被动，client主动：根据227响应的回复获得server的IP 
{
	/*************************************************************
	//3. code here
	*************************************************************/
	...
	for ( ; ; )
	{
		...
		if (FD_ISSET(STDIN_FILENO, &rset)) {
			...
			if (replycode==LOGIN || replycode==CLOSEDATA || replycode==PATHNAME || replycode==ACTIONOK)
			{
				...
				if (strncmp(rbuf, "ls", 2) == 0) {   //在两个字符串之间根据指定的字符串做匹配 
					sprintf(wbuf, "%s", "PASV\n");
					write(sockfd, wbuf, 5);
		...
		if (FD_ISSET(sockfd, &rset)) {
			...
			*************************************************************/
			...
			if (strncmp(rbuf, "227", 3) == 0) {  //在被动模式，server通过227相应来发送server的ip地址和端口号信息 
